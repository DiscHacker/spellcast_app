...
class Board:
    ...
    def play_move(self, move: SearchNode):
        tile_chain = [self.tile_at(node.x, node.y) for node in move.chain()]
        gem_count = sum(TileModifier.GEM in tile.modifiers for tile in tile_chain)
        ...
        if held_letter_boost is not None:
            choice(flat_packed_tiles).modifiers.add(held_letter_boost)
        
        available_tiles = set(flat_packed_tiles) - set(tile_chain)
        for _ in range(gem_count):
            if available_tiles:
                selected_tile = choice(list(available_tiles))
                selected_tile.modifiers.add(TileModifier.GEM)
                available_tiles.remove(selected_tile)
        ...
        choice(flat_packed_tiles).modifiers.add(TileModifier.DOUBLE_WORD)
